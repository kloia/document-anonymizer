...
class TestAnonymizeDocument:
    ...
    async def test_dry_run_mode(self, anonymizer):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch('document_anonymizer.document_anonymizer.get_pdf_info') as mock_info, \
                 patch('document_anonymizer.document_anonymizer.pdf_to_images') as mock_convert, \
                 patch.object(anonymizer.field_detector, 'detect_document_language', new_callable=AsyncMock) as mock_lang, \
                 patch.object(anonymizer.ocr_processor, 'run_ocr', new_callable=AsyncMock) as mock_ocr, \
                 patch.object(anonymizer.field_detector, 'detect_sensitive_fields', new_callable=AsyncMock) as mock_detect:
...
class TestAnonymizeBatch:
    ...
    async def test_batch_not_directory(self, anonymizer):
        ...
        result = await anonymizer.anonymize_batch(
            '/nonexistent/folder',
            '/tmp/output'
        )
        assert result == []
...
class TestClose:
    ...
    async def test_close_cleanup(self):
        ...
        with patch.object(anonymizer.field_detector, 'close', new_callable=AsyncMock) as mock_detector_close, \
    ...
    async def test_auto_approve_when_no_callback(self, anonymizer):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch('document_anonymizer.document_anonymizer.get_pdf_info') as mock_info, \
                 ...
                 patch.object(anonymizer.ocr_processor, 'run_ocr', new_callable=AsyncMock) as mock_ocr, \
                 patch.object(anonymizer.field_detector, 'detect_sensitive_fields', new_callable=AsyncMock) as mock_detect, \
                 patch.object(anonymizer.verifier, 'verify_masked_document', new_callable=AsyncMock) as mock_verify:

                mock_info.return_value = {'page_count': 1}
                mock_convert.return_value = [sample_image]
                mock_lang.return_value = {'languages': ['en'], 'locale': 'en_US'}
                mock_ocr.return_value = {'text_blocks': []}
                mock_detect.return_value = ([], [needs_review_field])  # auto_mask empty, needs_review has 1
