...
def is_signature_or_stamp(field: Dict) -> bool:
    ...
    signature_stamp_types = {
        'stamp', 'signature', 'seal', 'handwriting',
        'kase', 'imza', 'muhur'  # Turkish terms
    }
    is_signature_type = field_type in signature_stamp_types

    # LLM-detected signature/stamp (but not signatory_name, etc.)
    is_llm_signature = (
        ('signature' in field_type or 'stamp' in field_type) and
        'name' not in field_type and
        'number' not in field_type
    )
