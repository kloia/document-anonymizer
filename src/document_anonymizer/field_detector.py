...
class FieldDetector:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.auto_mask_confidence = detection_config.get('auto_mask_confidence', CONFIDENCE_AUTO_MASK)
    ...
    async def detect_sensitive_fields(
        ...
        if self.use_llm_classification:
            try:
                ...
                logger.info(f"Page {page_num}: Unified detection - {text_count} text, {visual_count} visual")
                ...
                if self.use_fallback_detection:
                    ...
                    if pattern_fields:
                        logger.info(f"Page {page_num}: Pattern supplement found {len(pattern_fields)} additional fields")
        ...
        logger.info(
            f"Page {page_num}: {len(auto_mask)} auto-mask, "
